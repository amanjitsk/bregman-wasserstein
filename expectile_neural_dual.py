...
from data import AbstractPairedSampler, IterableSampler
...
class ENOTPotentials(dual_potentials.DualPotentials):
    ...
    def __init__(
        ...
        *,
        ...
        def g_cost_conjugate(x: Array) -> Array:
    ...
    def transport(  # noqa: D102
        self, vec: Array, forward: bool = True
    ) -> Array:
...
class PotentialModelWrapper(potentials.BasePotential):
    ...
    def potential_gradient_fn(
        self, params: frozen_dict.FrozenDict[str, Array]
...
class ExpectileNeuralDual:
    ...
    def __init__(
        ...
        if use_dot_product:
            assert isinstance(self.cost_fn, costs.SqEuclidean), (
        ...
        self.state_f = self.neural_f.create_train_state(rng_f, optimizer_f, (dim_data,))
        self.state_g = self.neural_g.create_train_state(rng_g, optimizer_g, (dim_data,))
    ...
    def __call__(
        ...
        rng = utils.default_prng_key(None)
        rng_train, rng_valid = jax.random.split(rng, 2)
        logs = self.train_fn(
            IterableSampler(sampler, train_batch_size, rng_train).iterator(),
            IterableSampler(sampler, valid_batch_size, rng_valid).iterator(),
            callback=callback,
        )
    ...
    def train_fn(
        self,
        train_iterator: Iterator[Tuple[Array, Array]],
        valid_iterator: Iterator[Tuple[Array, Array]],
        callback: Optional[Callback_t] = None,
    ) -> Train_t:
        ...
        for step in tqdm(range(self.num_train_iters)):
            ...
            if update_forward:
                train_batch["source"], train_batch["target"] = next(train_iterator)
                (self.state_f, self.state_g, loss, loss_f, loss_g, w_dist) = (
                    self.train_step(self.state_f, self.state_g, train_batch)
                )
            else:
                train_batch["target"], train_batch["source"] = next(train_iterator)
                (self.state_g, self.state_f, loss, loss_g, loss_f, w_dist) = (
                    self.train_step(self.state_g, self.state_f, train_batch)
                )
            ...
            if step != 0 and step % self.valid_freq == 0:
                valid_batch["source"], valid_batch["target"] = next(valid_iterator)
    ...
    def _expectile_loss(self, diff: Array) -> Array:
    ...
    def _get_g_value_partial(
        self,
        params_g: frozen_dict.FrozenDict[str, Array],
        g_value: Callable[
            [frozen_dict.FrozenDict[str, Array]], potentials.PotentialValueFn_t
        ],
    ):
        if self.use_dot_product:
            g_value_partial = lambda y: -jax.vmap(g_value(params_g))(y)
            g_value_partial_detach = lambda y: -jax.vmap(
    ...
    def _distance(
        ...
        if self.use_dot_product:
            w_dist = (
                jnp.mean(jnp.sum(source**2, axis=-1))
    ...
    def _loss_fn(
        self,
        params_f: frozen_dict.FrozenDict[str, Array],
        params_g: frozen_dict.FrozenDict[str, Array],
        gradient_f: Callable[
            [frozen_dict.FrozenDict[str, Array]], potentials.PotentialGradientFn_t
        ],
        g_value: Callable[
            [frozen_dict.FrozenDict[str, Array]], potentials.PotentialValueFn_t
        ],
        ...
        g_star_source = batch_cost(source, target_hat_detach) - g_value_partial(
            target_hat_detach
        )

        diff_1 = (
            jax.lax.stop_gradient(g_star_source - batch_cost(source, target)) + g_target
        )
        reg_loss_1 = self._expectile_loss(diff_1).mean()

        diff_2 = (
            jax.lax.stop_gradient(g_target - batch_cost(source, target)) + g_star_source
        )
